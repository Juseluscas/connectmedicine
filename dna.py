...
for tenant in dna_data:
    if tenant["macAddress"] in myMacAddresses1:
        localidade_maca = (json.dumps(tenant["hierarchy"], indent=4, sort_keys=True))
    ...
    if tenant["macAddress"] in myMacAddresses3:
        localidade_cadeira3 = (json.dumps(tenant["hierarchy"], indent=4, sort_keys=True))
     #   print(localidade_cadeira3)

    if tenant["macAddress"] in myMacAddresses4:
        localidade_cadeira4 = (json.dumps(tenant["hierarchy"], indent=4, sort_keys=True))
      #  print(localidade_cadeira4)

    if tenant["macAddress"] in myMacAddresses5:
        localidade_cadeira5 = (json.dumps(tenant["hierarchy"], indent=4, sort_keys=True))
